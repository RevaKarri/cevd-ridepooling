...
class Environment(metaclass=ABCMeta):
    ...
    def __init__(self, NUM_LOCATIONS: int, MAX_CAPACITY: int, EPOCH_LENGTH: float, NUM_AGENTS: int, START_EPOCH: float, STOP_EPOCH: float, DATA_DIR: str, NUM_CLUSTERS: int):
        # Load environment
    ...
    def _get_rebalance_targets(self, agents: List[LearningAgent]) -> List[Request]:
        ...
        for _ in range(num_targets):
            target = random.choice(self.recent_request_history)
...
class NYEnvironment(Environment):
    ...
    def __init__(self, NUM_AGENTS: int, START_EPOCH: float, STOP_EPOCH: float, MAX_CAPACITY, NUM_CLUSTERS : int, DATA_DIR: str='../data/ny/', EPOCH_LENGTH: float = 60.0):
        super().__init__(NUM_LOCATIONS=self.NUM_LOCATIONS, MAX_CAPACITY=MAX_CAPACITY, EPOCH_LENGTH=EPOCH_LENGTH, NUM_AGENTS=NUM_AGENTS, START_EPOCH=START_EPOCH, STOP_EPOCH=STOP_EPOCH, DATA_DIR=DATA_DIR, NUM_CLUSTERS=NUM_CLUSTERS)
    ...
    def get_initial_states(self, num_agents: int, is_training: bool) -> List[int]:
        ...
        if is_training:
            ...
            for _ in range(num_agents):
                initial_state = random.randint(0, self.NUM_LOCATIONS - 1)
